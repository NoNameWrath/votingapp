<script>
  ...
  const usersendpoint = `http://localhost:1717/api/users?house=${house}&gender=${gender}`;
  const captainendpoint = `http://localhost:1717/api/vote?house=${house}&gender=${gender}`;
  ...
  const vote = async () => {
    if (!confirm) {
      ...
      const endpoint = `http://localhost:1717/api/vote`;
